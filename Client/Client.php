...
namespace Ekyna\Bundle\PayumSipsBundle\Client;
...
/**
 * Class Client
 * @package Ekyna\Bundle\PayumSipsBundle\Client
...
class Client
{
    ...
    protected function handleRequestOutput($output)
    {
        ...
        if ('' === $code && '' === $error) {
            throw new RuntimeException(sprintf('SIPS Request failed. Output: %s', $output));
        } elseif ('0' !== $code) {
            throw new RuntimeException(sprintf('SIPS Request failed with the following error: %s', $error));
        }

        if (!$message) {
            throw new RuntimeException(sprintf('SIPS Output message missing. Output: %s', $output));
        }
